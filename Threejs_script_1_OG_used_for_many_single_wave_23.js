...
function parseCountsToGrid(counts) {
  ...
  for (const [bitstring, count] of Object.entries(counts)) {
    const u = parseInt(bitstring.slice(3), 2);
    const v = parseInt(bitstring.slice(0, 3), 2);
    matrix[u][v] = count;
...
function createWaveSurface(matrix, maxCount, xOffset) {
  ...
  for (let x = 0; x < 8; x++) {
    for (let y = 0; y < 8; y++) {
      ...
      dot.position.set(x - 3.5 + xOffset, 0, y - 3.5);
      redDots.push(dot);
      scene.add(dot);
    }
  }
...
// 10. Load all three input files
Promise.all([
  loadQuantumData('ECDLP_8pts_Shors_Run_0.json'),
  loadQuantumData('ECDLP_8pts_Shors_Run_Bloch_Clock_0.json'),
  loadQuantumData('Shors_ECC_3_Bit_Key_Two_Bloch_Clocks_0.json')
]).then(([data1, data2, data3]) => {
  const { matrix: matrix1, maxCount: max1 } = parseCountsToGrid(data1);
  const { matrix: matrix2, maxCount: max2 } = parseCountsToGrid(data2);
  const { matrix: matrix3, maxCount: max3 } = parseCountsToGrid(data3);

  createWaveSurface(matrix1, max1, -20);  // Left wave
  createWaveSurface(matrix2, max2,   0);  // Center wave
  createWaveSurface(matrix3, max3, +20);  // Right wave
