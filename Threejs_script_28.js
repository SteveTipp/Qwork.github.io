...
// Threejs 6-bit break full interference map
// Config 
const HALF_BITS = 6;                 // 6 per axis 
const TOTAL_BITS = 2 * HALF_BITS;    // 12
const GRID = 1 << HALF_BITS;         // 64
const MOD = GRID;                    // 64
const K = 42;                        // target k and ridge slope (u + K v ≡ 0 mod 64)
...
function parseEntries(counts) {
  ...
  for (const [bit, cRaw] of Object.entries(counts)) {
    ...
    const left  = b12.slice(0, HALF_BITS);    // raw left 6
    const right = b12.slice(HALF_BITS);       // raw right 6
...
// Reproduce the circuit's top 100 invertible (a,b) pairs and recovered k from backend counts (JSON)
function selectTop3ByClassical(counts, targetK = K, nBits = HALF_BITS) {
  const MOD = 1 << nBits;
  ...
  for (const e of sortedAll) {
    if ((e.b & 1) === 0) continue;                 // gcd(b,2^n)!=1 -> skip
    const bInv = invModPow2(e.b, nBits);
    if (bInv == null) continue;
    const k = ((MOD - (e.a % MOD)) * bInv) % MOD;  // k ≡ -(a)*b^{-1} mod 2^n
  ...
  console.log('Selected from classical top-100 (6-bit backend):', picks.map(e => ({
...
function createWaveSurface(matrix, maxCount, highlightSet) {
  ...
  for (let y = 0; y < gridY; y++) {
    for (let x = 0; x < gridX; x++) {
      ...
      const color = (isRidge && amp > threshold)
        ? new THREE.Color(0x0000ff) // ridge cells (blue), using true backend amplitude
  ...
  // Yellow spheres at the classically selected (u,v), taken from real backend counts
...
loadQuantumData().then(counts => {
  ...
  // Build surface from all counts using (u,v) from 6-bit backend json
  const entries = parseEntries(counts);
